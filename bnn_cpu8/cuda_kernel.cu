...
float layer3_step_cuda(float * cuda_layer_2_output, unsigned long long * cuda_layer_3_output){
    ...
    const int GRIDXSIZE = 1;//(DATAXSIZE+BLKXSIZE-1)/BLKXSIZE; // 4
    const int GRIDYSIZE = 1;//(DATAYSIZE+BLKYSIZE-1)/BLKYSIZE; // 4
    // const int GRIDZSIZE = 64;//(DATAZSIZE+BLKZSIZE-1)/BLKZSIZE; // 1
...
__global__ void layer8_gemm_kernel(unsigned long long *d_cuda_layer_7_output, float *d_layer_8_bias, unsigned long long *d_cuda_layer_8_weight, signed short *d_cuda_layer_8_output){
    ...
    if(d < 2048){
        ...
        for (int i = 0; i < 49; i++) {
            d_cuda_layer_8_output[d] += 2 * __popcll((unsigned long long)~(unsigned long long)(d_cuda_layer_8_weight[d*49+i] ^ d_cuda_layer_7_output[b*49+i])) - 64;
        }
    }
}
...
__global__ void layer10_gemm_kernel(unsigned long long *d_cuda_layer_9_output, float *d_layer_10_bias, unsigned long long *d_cuda_layer_10_weight, signed short *d_cuda_layer_10_output){
    ...
    if(d < 10){
        ...
        for (int i = 0; i < 32; i++) {
            d_cuda_layer_10_output[d] += 2 * __popcll((unsigned long long)~(unsigned long long)(d_cuda_layer_10_weight[d*32+i] ^ d_cuda_layer_9_output[b*32+i])) - 64;
        }
    }
}
