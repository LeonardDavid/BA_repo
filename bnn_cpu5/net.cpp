...
int index(const int x, const int y, const int z) {
  ...
  void predict_NeuralNet(unsigned char * const x, float * pred, int c) {
      ...
      // int x_c;
      // x_c = cuda_k(layer_4_output);
      // std::cout<<x_c<<std::endl;

      //signed short* b[DATAYSIZE][DATAXSIZE] = { { (signed short *) malloc(sizeof(signed short) * DATAZSIZE) } };

      // signed short* b[14][64];

      std::cout<<"----------begin----------"<<std::endl;

      signed short xa[DATAXSIZE][DATAYSIZE][DATAZSIZE];
      signed short xc[DATAXSIZE][DATAYSIZE][DATAZSIZE];
      ...
      std::cout<<"populate & flatten xa to xb:"<<std::endl<<std::endl;
      for(int i=0; i<DATAXSIZE; i++){
        for(int j=0; j<DATAYSIZE; j++){
          for(int k=0; k<DATAZSIZE; k++){
            xa[i][j][k] = i+j+k;
            xb[index(i,j,k)] = xa[i][j][k];
            // xb[index(i,j,k)] = i+j+k;
          }
        }
      }
      ...
      // deflatten xb to xc
      std::cout<<"deflatten xb to xc:"<<std::endl;
      for(int i=0; i<DATAXSIZE; i++){
        for(int j=0; j<DATAYSIZE; j++){
          for(int k=0; k<DATAZSIZE; k++){
            xc[i][j][k] = xb[index(i,j,k)];
          }
        }
      }

      // check xa cout
      std::cout<<"check xa:"<<std::endl;
      for(int i=0; i<DATAXSIZE; i++){
        for(int j=0; j<DATAYSIZE; j++){
          for(int k=0; k<DATAZSIZE; k++){
            std::cout<<xa[i][j][k]<<" ";
