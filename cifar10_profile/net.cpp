...
//unsigned char (*layer_0_output)[32][3] = (unsigned char (*)[32][3]) x;
  ...
  for (int b = 0; b < BATCH_SIZE; b++){
    for (int h = 0; h < 32; h++) {
      for (int w = 0; w < 32; w++) {
        for (int m = 0; m < 128; m++) {
          cuda_layer_3_output[index4D(b,h,w,m,32,32,128)] = layer_3_bias[m];
        }
        for (int kH = 0; kH < 3; kH++) {
          int iH = h * 1 + kH - 1;
          if (iH >= 0 && iH < 32) {
            for (int kW = 0; kW < 3; kW++) {
              ...
              if (iW >= 0 && iW < 32) {
                for (int m = 0; m < 128; m++) {
                  for (int c = 0; c < 2; c++) {
                    cuda_layer_3_output[index4D(b,h,w,m,32,32,128)] += 2 * __builtin_popcountll((unsigned long long)~(unsigned long long)(layer_3_weight[kH][kW][m][c] ^ cuda_layer_2_output[index4D(b,iH,iW,c,32,32,2)])) - 64; // layer_2_output[b][iH][iW][c] / cuda_layer_2_output[index4D(b,iH,iW,c,32,32,2)]
                  }
                }
              }
            }
  ...
  for (int b = 0; b < BATCH_SIZE; b++){
    for (int h = 0; h < 16; h++) {
      for (int w = 0; w < 16; w++) {
        for (int m = 0; m < 256; m++) {
          cuda_layer_6_output[index4D(b,h,w,m,16,16,256)] = layer_6_bias[m];
        }
        for (int kH = 0; kH < 3; kH++) {
          int iH = h * 1 + kH - 1;
          if (iH >= 0 && iH < 16) {
  ...
  for (int b = 0; b < BATCH_SIZE; b++){
    for (int h = 0; h < 16; h++) {
      for (int w = 0; w < 16; w++) {
        for (int m = 0; m < 256; m++) {
          cuda_layer_8_output[index4D(b,h,w,m,16,16,256)] = layer_8_bias[m];
        }
        for (int kH = 0; kH < 3; kH++) {
          int iH = h * 1 + kH - 1;
          if (iH >= 0 && iH < 16) {
  ...
  for (int b = 0; b < BATCH_SIZE; b++){
    for (int h = 0; h < 8; h++) {
      for (int w = 0; w < 8; w++) {
        for (int m = 0; m < 512; m++) {
          cuda_layer_11_output[index4D(b,h,w,m,8,8,512)] = layer_11_bias[m];
        }
        for (int kH = 0; kH < 3; kH++) {
          int iH = h * 1 + kH - 1;
          if (iH >= 0 && iH < 8) {
  ...
  for (int b = 0; b < BATCH_SIZE; b++){
    for (int h = 0; h < 8; h++) {
      for (int w = 0; w < 8; w++) {
        for (int m = 0; m < 512; m++) {
          cuda_layer_13_output[index4D(b,h,w,m,8,8,512)] = layer_13_bias[m];
        }
        for (int kH = 0; kH < 3; kH++) {
          int iH = h * 1 + kH - 1;
          if (iH >= 0 && iH < 8) {
