...
float layer3_conv_cuda(unsigned long long * cuda_layer_2_output, float * cuda_layer_3_output){
    ...
    cudaMalloc((void **) &d_cuda_layer_3_weight, 3*3*128*2*sizeof(unsigned long long)); //  = 3x3x128x2 dim of layer_3_weight (without x64 as it would lead to segmentation fault at memcpy (even though there are bigger array mallocs))
