...
auto benchmark(vector<MNISTLoader> &loaderx, bool verbose = false) {
    ...
    // Only executes first 4 batches of each section
    int lsize = loaderx[0].size(); // 10000
    int sections = 10*BATCH_SIZE;
    int secsize = lsize/sections; // 1000/BATCH_SIZE
    // max lsize for 1 batch: 1904
    // max lsize for n batches: floor(1904/n)
    printf("lsize: %dx%d\nsecsize: %dx%dx%d\n", BATCH_SIZE, lsize, BATCH_SIZE, sections, secsize);

    for(int sec=0; sec<sections; sec++){
    
        cudnnHandle_t handle_;
        cudnnCreate(&handle_);
        cout << "Created cuDNN handle " << sec << endl << endl;

        // create the tensor descriptor
        cudnnDataType_t dtype = CUDNN_DATA_FLOAT;
        cudnnTensorFormat_t format = CUDNN_TENSOR_NHWC;
        int n = secsize, h = 28, w = 28, c = 1;
        ...
        for(int b=0; b<BATCH_SIZE; b++){        // b := # batch
            for(int i=sec*secsize; i<(sec+1)*secsize; i+=factor){ // i := # image
                for(int p=0; p<imgsize; p++){   // p := # pixel
                    img[index3D(b,(i-sec*secsize),p,secsize,imgsize)] = loaderx[b].images(i)[p]; //  loaderx[b].images(i)[p];
                }
        ...
        for(int b=0;b<BATCH_SIZE;b++){
            ...
            for(int im=sec*secsize; im<(sec+1)*secsize; im+=factor){
                for (int i = 0; i < 28; i++)
                {
                    for (int j = 0; j < 28; j++)
                    {
                        // printf("%d ", img[index4D(b,im,i,j,lsize,28,28)]);
                        g<<int(img[index4D(b,(im-sec*secsize),i,j,secsize,28,28)])<<" ";
                    }
                    // printf("\n");
                    g<<"\n";
                }
        ...
        total_gpu_time += static_cast<float>(std::chrono::duration_cast<std::chrono::milliseconds>(endgpu-startgpu).count());

        // handle has to be destroyed before accesing img
        cudnnDestroy(handle_);
        cout << endl << "Destroyed cuDNN handle " << sec << endl << endl;
        ...
        for(int b=0;b<BATCH_SIZE;b++){
            ...
            for(int im=sec*secsize; im<(sec+1)*secsize; im+=factor){
                for (int i = 0; i < 28; i++)
                {
                    ...
                    {
                        // printf("%d ", img[index4D(b,im,i,j,lsize,28,28)]);
                        gg<<int(img[index4D(b,(im-sec*secsize),i,j,secsize,28,28)])<<" ";
                    }
                    // printf("\n");
                    gg<<"\n";
                }
